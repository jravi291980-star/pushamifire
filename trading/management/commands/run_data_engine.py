...
class Command(BaseCommand):
    help = 'Runs Fyers V3 Data Socket with Token Refresh'
    ...
    def handle(self, *args, **options):
        while True:
            ...
            def on_error(msg):
                ...
                # If 403 or Invalid Symbol limit reached, it might be token issue
                if '403' in str(msg):
                    logger.critical("Token Expired. Restarting...")
                    # Allow loop to restart

            def on_open():
                logger.info(f"Connected. Subscribing to {len(symbols)} symbols in batches...")
                batch_size = 50
                for i in range(0, len(symbols), batch_size):
                    batch = symbols[i:i+batch_size]
                    fyers_socket.subscribe(symbols=batch, data_type="SymbolUpdate")
                    time.sleep(0.5) # Increased delay to be safe
                fyers_socket.keep_running()

            try:
                fyers_socket = data_ws.FyersDataSocket(
                    access_token=token, log_path="", litemode=False, write_to_file=False,
                    reconnect=True, on_connect=on_open, on_error=on_error, on_message=on_message
                )
                fyers_socket.connect()
            except Exception as e:
                logger.error(f"Engine Crashed: {e}")
            
            logger.info("Restarting Data Engine Sequence...")
            time.sleep(3)